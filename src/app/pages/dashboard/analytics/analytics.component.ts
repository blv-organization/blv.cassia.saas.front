...
export class AnalyticsComponent implements AfterViewInit {
  ...
  defaultDate = [new Date(2025, 3, 1), new Date(2026, 2, 2)];
    ...
    flatpickr(this.datepicker.nativeElement, {
      ...
      onReady: (selectedDates: Date[], dateStr: string, instance: Instance) => {
        ...
        if(selectedDates[0] && selectedDates[1]){
          this.getQuotationRanking(selectedDates[0], selectedDates[1]);
        }
      },
  ...
  private getQuotationRanking(from: Date, to: Date) {
    const formattedFrom = from.toISOString().split('T')[0];;
    const formattedTo = to.toISOString().split('T')[0];;
    this.analyticService.getQuotationRanking(formattedFrom, formattedTo)
      .subscribe(res => {
        console.log(res);
      });
  }
